...
export default function AvatarSection({ avatar }) {
  ...
  const normalizeSlotName = (slotName) => {
    if (!slotName) return "알 수 없음";
    return slotName.replace(/ 아바타$/, ""); // " 아바타" 제거
  };

  // API 응답의 slotName을 기준으로 정렬
  const sortedAvatar = [...avatar].sort((a, b) => {
    const orderA = slotOrder.indexOf(normalizeSlotName(a.slotName));
    const orderB = slotOrder.indexOf(normalizeSlotName(b.slotName));
    ...
    <div className="mb-8 p-6 bg-white dark:bg-neutral-800 shadow-xl rounded-lg border border-neutral-200 dark:border-neutral-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {sortedAvatar.map((item, index) => {
          const displaySlotName = normalizeSlotName(item.slotName);
          // API 응답에서 item.itemName이 없고 item.clone.itemName을 사용해야 함
          const displayItemName = item.clone?.itemName || item.itemName || "정보 없음";
          // itemImage는 API 응답에 없으므로, Image 컴포넌트는 onError로 처리될 것임
          // 또는 item.itemId (API에 있다면) 를 사용해 이미지 URL을 동적으로 구성해야 함
          // 여기서는 itemImage가 없다고 가정하고 onError에 의존

          return (
            <div key={index} className="p-3 bg-neutral-50 dark:bg-neutral-700 rounded shadow flex flex-col">
              <div className="flex items-center mb-2">
                {item.itemImage && ( // itemImage가 실제로는 없지만, 구조는 유지
                ...
                <div>
                  <p className="font-semibold text-neutral-700 dark:text-neutral-300">{displaySlotName}</p>
