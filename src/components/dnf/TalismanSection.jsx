          ...
          return (
            <div key={index} className="p-4 bg-neutral-50 dark:bg-neutral-700 rounded shadow">
              <div className="flex items-center mb-3">
                {mainTalisman?.itemImage && ( // itemImage는 API 응답에 없지만 구조 유지
                  <div className="relative w-12 h-12 mr-3 flex-shrink-0">
                    <Image
                      src={mainTalisman.itemImage}
                      alt={mainTalisman.itemName || '탈리스만 이미지'}
                      width={48}
                      height={48}
                ...
                <div>
                  <p className={`text-lg font-semibold ${mainTalisman?.itemRarity ? rarityColorMap[mainTalisman.itemRarity] : 'text-neutral-800 dark:text-neutral-100'}`}>
                    {mainTalisman?.itemName || "탈리스만 정보 없음"}
                  </p>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {runes.map((rune, runeIndex) => (
                      <div key={runeIndex} className="p-2 bg-neutral-100 dark:bg-neutral-600 rounded text-center flex flex-col items-center">
                        {rune.itemImage && ( // rune.itemImage는 API 응답에 없지만 구조 유지
                        ...
                        <p className={`text-xs font-medium ${rune.itemRarity ? rarityColorMap[rune.itemRarity] : 'text-neutral-700 dark:text-neutral-200'}`}>
