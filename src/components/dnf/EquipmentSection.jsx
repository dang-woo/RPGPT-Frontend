  ...
  if ((!equipment || equipment.length === 0) && (!setItemInfo || setItemInfo.length === 0)) {
    return (
      <div className="mb-8 p-6 bg-white dark:bg-neutral-800 shadow-xl rounded-lg border border-neutral-200 dark:border-neutral-700">
        <h2 className="text-2xl font-bold mb-4 text-[var(--link-accent-color)]">장착 장비 및 세트 효과</h2>
        <p className="text-neutral-500 dark:text-neutral-400">표시할 장비 또는 세트 아이템 효과가 없습니다.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mb-8 p-6 bg-white dark:bg-neutral-800 shadow-xl rounded-lg border border-neutral-200 dark:border-neutral-700">
      ...
      {setItemInfo && setItemInfo.length > 0 && (
        <div className="mb-6 pb-4 border-b border-neutral-200 dark:border-neutral-600">
          <button 
            onClick={() => setIsSetEffectVisible(!isSetEffectVisible)}
            className="flex items-center justify-between w-full py-2 px-1 text-lg font-semibold text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded-md transition-colors"
          >
        ...
        <ul className="space-y-4">
          {sortedEquipment.map((item, index) => {
            const itemNameClass = item.itemRarity === '태초' && item.itemName && item.itemName.includes('흑아')
                                  ? 'text-red-500'
                                  : rarityColorMap[item.itemRarity] || 'text-neutral-800 dark:text-neutral-100';
            return (
              <li key={index} className="p-4 bg-neutral-50 dark:bg-neutral-700 rounded-md shadow-sm border border-neutral-200 dark:border-neutral-600 flex gap-4">
                ...
                <div className="flex-grow">
                  ...
                  <p className={`text-lg sm:text-xl font-medium ${itemNameClass} mb-1.5`}>
                    {item.itemName}
